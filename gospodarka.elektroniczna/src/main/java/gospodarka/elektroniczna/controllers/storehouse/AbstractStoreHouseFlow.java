...
import gospodarka.elektroniczna.documents.storehouse.AbstractStoreHouseDocument;
import gospodarka.elektroniczna.dto.storehouse.AbstractStoreHouseUser;
...
public abstract  class AbstractStoreHouseFlow<T extends AbstractStoreHouseUser> {
	    ...
	    public AbstractStoreHouseFlow(String documentTitle, DocumentTypes documentType, Departments targetDepartment) {
	    ...
	    protected boolean submit(UserData userData, Departments targetDepartment, T documentData) {
	        try {
	            ...
	            Document<AbstractStoreHouseDocument<T>> document = documentService.createDocument(documentType, documentTitle,
	                    senderDepartment);
	        ...
	        LoggerFactory.getLogger(PPZDocument.class).error("błąd", e);
	            return false;
	        }
	    }
	    ...
	    protected List<T> search(SearchCriteria criteria) {
	        ...
	        for (DocumentStub documentStub : documentStubs) {
	            Document<AbstractStoreHouseDocument<T>> document = documentService.loadCurrentDocument(documentStub);
